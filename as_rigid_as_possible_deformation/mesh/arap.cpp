...
PetscErrorCode Deformation::destroy()
{
...
PetscErrorCode Deformation::assembleMat()
{
...
PetscErrorCode Deformation::assembleRHS()
{
	const int n_col = _mesh->numberOfVertices();
	const int n_row = _mesh->numberOfVertices()+_anchors.size();
...
PetscErrorCode  Deformation::updateRots()
{
...
PetscErrorCode Deformation::deform(vector<int> anchorverts, vector<float> newpos)
{
...
PetscErrorCode Deformation::resetMeshPositions()
{
	for (auto iv = _mesh->getVertices()->begin() ; iv != _mesh->getVertices()->end() ; ++iv)
	{
		Vertex *v = *iv;
